...
typedef enum component_type { REGULAR_CHAR, ESCAPE_BACKSLASH, DOT, SQUARED_BRACKETS, CIRCULAR_BRACKETS } component_type;
...
int is_match_in_line(char *haystack, char *needle, struct arguments *params, phrase_component *components_list,
                     ...
                     int components_count)
{
  ...
  int match = 0, needle_len = strlen(needle), haystack_len = (strlen(haystack)), component_index = 0,
      haystack_index = 0;
  ...
  if (params->x) {
    ...
    for (; haystack_index < haystack_len; haystack_index++) {
      if ((match = is_match_at_place(haystack + haystack_index, needle, component_index, components_list,
                                     components_count, params))) {
        break;
      }
    }
  }
  ...
  if (component_index >= component_count) {
    match = TRUE;
             ...
             (component_list[component_index].type == SQUARED_BRACKETS &&
              component_list[component_index].range_start <= haystack[0] &&
    ...
    while (current_string_index < component_end_index) {
      ...
      if (strncmp(needle + current_string_index - compare_length, haystack, compare_length) == 0) {
        match = is_match_at_place(haystack + compare_length, needle, component_index + 1, component_list,
  ...
  for (ind = 1; ind < argc; ind++) {
    if (argv[ind][0] != '-' && *phrase == NULL) {
      *phrase = argv[ind];
      ...
      params->A = 1;
      params->NUM = (int)strtoul(argv[++ind], NULL, DECIMAL_BASE);
